...
- Data: 400 train / 52 eval single-turn examples derived from
  `glaiveai/glaive-function-calling-v2` (Apache 2.0): system message lists tool
  schemas; assistant answers with a JSON call `{"name":..., "arguments":...}` —
  or a plain answer for the ~12% no-tool cases (to measure false tool-calls).
...
- Training: LoRA r=16, α=32, all-linear, assistant-only loss masking, 1 epoch,
  bf16, on Apple MPS (`train_lora.py`).
- Eval (`eval_tools.py`), greedy: parse rate, function-name accuracy, argument
  exact-match (40 call cases), false-call rate (12 no-call cases).
...
| **+ LoRA (this work)** | **92.5%** | **92.5%** | **90.0%** | 41.7% |
