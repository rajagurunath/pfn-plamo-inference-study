...
truncated on token rejection the way a KV cache can, and vLLM currently
rejects all speculative-decoding methods on Mamba-hybrid models outright.
llama.cpp recently added a generic SSM-state checkpoint/rollback mechanism,
making it---to our knowledge---the first practical platform to test
speculative decoding on such models. We benchmark
\code{pfnet/plamo-2-8b} (target) with \code{pfnet/plamo-2-1b} (draft) on an
Apple M4~Pro (Metal backend, llama.cpp build b9596) across three task
families with greedy sampling, verifying byte-identical outputs versus the
non-speculative baseline. The result is a carefully measured
...
truncation operator. As of this writing, vLLM raises a
\code{NotImplementedError} (``Mamba with speculative decoding is not
supported yet'') for \emph{all} speculative methods on Mamba-hybrid
models~\cite{vllmissue}.

llama.cpp recently solved rollback generically via per-sequence rolling
checkpoints of SSM state~\cite{llamacpppr}, opening a window to ask an
empirical question that, to our knowledge, has not been answered for
PLaMo~2: \emph{does speculative decoding actually pay off on a hybrid
SSM--attention model, once correctness is solved?} We answer it for the
PLaMo~2 8B/1B pair on Apple Silicon, with a rigorous losslessness check, and
report an honest negative result with a mechanistic explanation. As a
...
of the state. This is exactly why vLLM's speculative-decoding path refuses
Mamba-hybrid models for every proposer type (n-gram, draft model, EAGLE,
Medusa, MTP)~\cite{vllmissue}.
...
PLaMo~2 GGUFs published to date. A natural upstream fix is to protect
\code{ssm\_out} in llama.cpp's plamo2 quantization recipe. All benchmarks in
...
\paragraph{Implications.} For vLLM, these numbers temper the urgency of
lifting the Mamba-hybrid speculative-decoding
restriction~\cite{vllmissue}:
...
GitHub issue \#30114, vllm-project/vllm.
\url{https://github.com/vllm-project/vllm/issues/30114}

\bibitem{llamacpppr}
ggml-org/llama.cpp.
Recurrent state checkpointing for speculative decoding
(\code{llama\_memory\_recurrent} rolling checkpoint buffer, depth 8 per
sequence). Pull request \#20075.
\url{https://github.com/ggml-org/llama.cpp/pull/20075}
